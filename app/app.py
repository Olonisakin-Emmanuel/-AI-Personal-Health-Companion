...
def run_prediction():
    ...
    if disease is None or conf < 50:
        try:
            client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
            ai_prompt = f"Based on these symptoms: {', '.join(user_symptoms)}. Suggest 1–3 possible diseases and a short safe health tip in {lang_for_ai}."
